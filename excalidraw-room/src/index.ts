...
try {
  ...
  // Return the list of rooms and user count
  app.get("/rooms/", (req, res) => {
    const rooms: Map<string, Set<string>> = io.of("/").adapter.rooms;
    const users = new Set(
      Array.from(rooms.values()).flatMap((users) => Array.from(users)),
    );
    const roomsList = Array.from(rooms.entries())
      .filter(([roomID, ,]) => !users.has(roomID))
      .map(([roomID, users]) => [roomID, users.size]);
