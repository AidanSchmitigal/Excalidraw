/**
 * Coppied from https://github.com/zaaack/keyv-file/blob/master/index.ts
 * MIT License
 *
 * With changes to use no dependencies
 */
...
export class KeyvFile<V = any> {
  ...
  saveToDisk() {
    ...
    return new Promise<void>((resolve, reject) => {
      fs.mkdir(
        path.dirname(this._opts.filename),
        { recursive: true },
        (err) => {
          if (err) {
            reject(err);
      ...
      fs.writeFile(this._opts.filename, data, (err) => {
        if (err) {
          reject(err);
        } else {
          resolve();
