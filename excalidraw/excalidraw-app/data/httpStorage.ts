...
export const saveToHttpStorage = async (
  ...
  const storedElements = getSyncableElements(
    restoreElements(reconciledElements, null),
  );
...
const decryptElements = async (
  ...
  try {
    await decryptData(iv, ciphertext, roomKey);
  } catch (error: any) {}
...
export const getRoomsFromHttpStorage = async (): Promise<string[]> => {
  const response = await fetch(`${HTTP_STORAGE_BACKEND_URL}/rooms`);
