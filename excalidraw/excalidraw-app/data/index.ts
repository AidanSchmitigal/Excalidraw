...
export const RE_COLLAB_LINK = /^#room=([a-zA-Z0-9_-]+),([a-zA-Z0-9_-]+)$/;

export const isCollaborationLink = (link: string): boolean => {
  const hash = new URL(link).hash;
  const match = hash.match(RE_COLLAB_LINK);
  return match != null && match[2].length === ROOM_PASSWORD_LENGTH;
};

export const getCollaborationLinkData = (link: string) => {
  const hash = new URL(link).hash;
  const match = hash.match(RE_COLLAB_LINK);
  if (match && match[2].length !== ROOM_PASSWORD_LENGTH) {
    window.alert(
      t("alerts.invalidEncryptionKey", { length: ROOM_PASSWORD_LENGTH }),
    );
    return null;
  }
...
export const exportToBackend = async (
  ...
  try {
    ...
    if (json.id) {
      ...
      await getStorageBackend().saveFilesToStorageBackend({
